...
export class GiftTemplateController {
  ...
  async create(
    ...
    @Put(':id')
    @UseInterceptors(FileInterceptor('image'))
    async update(@Param('id', ParseIntPipe) id: number,@Query('lang') lang=Language.en, @Body() body: CreateGiftTemplateDto, @UploadedFile() file: Express.Multer.File) {
    ...
    return this.giftTemplateService.findByCategoryWithPagination(+categoryId, +page, +limit);
  }
}